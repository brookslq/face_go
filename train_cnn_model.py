...
from load_face_dataset import load_dataset, resize_iamge, size
...
class Model:
    ...
    # 训练模型
    def train(self, dataset, batch_size=120, nb_epoch=10, data_augmentation=True):
    ...
    def face_predict(self, image):
        # 依然是根据后端系统确定维度顺序
        if K.image_dim_ordering() == 'th' and image.shape != (1, 3, size, size):
            image = resize_iamge(image)  # 尺寸必须与训练集一致都应该是IMAGE_SIZE x IMAGE_SIZE
            image = image.reshape((1, 3, size, size))  # 与模型训练不同，这次只是针对1张图片进行预测
        elif K.image_dim_ordering() == 'tf' and image.shape != (1, size, size, 3):
            image = resize_iamge(image)
